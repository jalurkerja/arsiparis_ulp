...
<?php
	if($_GET["deleting"]){
		...
		if($db->fetch_single_data("ba_nego","id",array("procurement_work_id"=>$procurement_work_id)) == ""){
			$db->addtable("pokja_ulp");			$db->where("id",$_GET["deleting"]);
			$db->addfield("nego_nomor");		$db->addvalue("");
			$db->addfield("nego_tanggal");		$db->addvalue("");
			$db->addfield("nego_supplier_ids");	$db->addvalue("");
			$db->addfield("nego_undang_tgl");	$db->addvalue("");
			$db->addfield("nego_undang_jam");	$db->addvalue("");
			$db->addfield("nego_undang_tempat");$db->addvalue("");
			$db->addfield("nego_updated_at");	$db->addvalue("");
			$db->addfield("nego_updated_by");	$db->addvalue("");
			$db->update();
...
?>
<div class="bo_title">Undangan Negosiasi</div>
	...
	<div id="bo_filter_container">
		<?=$f->start("filter","GET");?>
			...
			<?php
				...
				$nego_nomor = $f->input("nego_nomor",$_GET["nego_nomor"]);
				$nego_tanggal = $f->input("nego_tanggal",$_GET["nego_tanggal"],"type='date'");
			?>
			     <?=$t->row(array("Nama Pekerjaan Pengadaan",$procurement_work_id));?>
			     <?=$t->row(array("Nomor Undangan",$nego_nomor));?>
			     <?=$t->row(array("Tanggal Undangan",$nego_tanggal));?>
...
<?php
	$whereclause = "nego_nomor <> '' AND ";
	if(@$_GET["procurement_work_id"]!="") 	$whereclause .= "procurement_work_id = '".$_GET["procurement_work_id"]."' AND ";
	if(@$_GET["nego_nomor"]!="") 	$whereclause .= "nego_nomor LIKE '%".$_GET["nego_nomor"]."%' AND ";
	if(@$_GET["nego_tanggal"]!="") 	$whereclause .= "nego_tanggal LIKE '%".$_GET["nego_tanggal"]."%' AND ";
...
?>
	...
	<?=$f->input("add","Tambah","type='button' onclick=\"window.location='pokja_ulp_nego_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($pokja_ulp as $no => $_pokja_ulp){ ?>
		<?php
			$actions = "<a href=\"pokja_ulp_nego_edit.php?id=".$_pokja_ulp["id"]."\">Ubah</a> |
		...
		<?=$t->row(
					array($actions,
						$no+$start+1,
						"<a href=\"pokja_ulp_nego_edit.php?id=".$_pokja_ulp["id"]."\">".$_pokja_ulp["id"]."</a>",
						"<a href=\"pokja_ulp_nego_edit.php?id=".$_pokja_ulp["id"]."\">".$_pokja_ulp["nego_nomor"]."</a>",
						"<a href=\"pokja_ulp_nego_edit.php?id=".$_pokja_ulp["id"]."\">".format_tanggal($_pokja_ulp["nego_tanggal"])."</a>",
						"<a href=\"pokja_ulp_nego_edit.php?id=".$_pokja_ulp["id"]."\">".$work_category."</a>",
                        "<a href=\"pokja_ulp_nego_edit.php?id=".$_pokja_ulp["id"]."\">".$procurement_work."</a>",
                        $ppk_name."<br>".$ppk_nip,
                        $tahun_anggaran,
                        $work_days,
