...
<?php
	if($_GET["deleting"]){
		...
		$db->addfield("ba_bayar_nomor");		$db->addvalue("");
		$db->addfield("ba_bayar_tanggal");		$db->addvalue("");
		$db->addfield("ba_bayar_nama");			$db->addvalue("");
		$db->addfield("ba_bayar_nip");			$db->addvalue("");
		$db->addfield("ba_bayar_bank");			$db->addvalue("");
		$db->addfield("ba_bayar_cabang");		$db->addvalue("");
		$db->addfield("ba_bayar_norek");		$db->addvalue("");
		$db->addfield("ba_bayar_updated_at");	$db->addvalue("");
		$db->addfield("ba_bayar_updated_by");	$db->addvalue("");
		$db->update();
		?> <script> window.location="?";</script> <?php
	}
?>
<div class="bo_title"> Berita Acara Pembayaran</div>
	...
	<div id="bo_filter_container">
		<?=$f->start("filter","GET");?>
			...
			<?php
				...
				$ba_bayar_nomor = $f->input("ba_bayar_nomor",$_GET["ba_bayar_nomor"]);
				$ba_bayar_tanggal = $f->input("ba_bayar_tanggal",$_GET["ba_bayar_tanggal"],"type='date'");
			?>
			     <?=$t->row(array("Nama Pekerjaan Pengadaan",$procurement_work_id));?>
			     <?=$t->row(array("Nomor Dokumen Berita Acara",$ba_bayar_nomor));?>
			     <?=$t->row(array("Tanggal Dokumen",$ba_bayar_tanggal));?>
...
<?php
	$whereclause = "ba_bayar_nomor <> '' AND ";
	if(@$_GET["procurement_work_id"]!="") 	$whereclause .= "procurement_work_id = '".$_GET["procurement_work_id"]."' AND ";
	if(@$_GET["ba_bayar_nomor"]!="") 	$whereclause .= "ba_bayar_nomor LIKE '%".$_GET["ba_bayar_nomor"]."%' AND ";
	if(@$_GET["ba_bayar_tanggal"]!="") 	$whereclause .= "ba_bayar_tanggal LIKE '%".$_GET["ba_bayar_tanggal"]."%' AND ";
...
?>
	...
	<?=$f->input("add","Tambah","type='button' onclick=\"window.location='spk_ba_bayar_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($spk as $no => $_spk){ ?>
		<?php
			$actions = "<a href=\"spk_ba_bayar_edit.php?id=".$_spk["id"]."\">Ubah</a> |
		...
		<?=$t->row(
					array($actions,
						$no+$start+1,
						"<a href=\"spk_ba_bayar_edit.php?id=".$_spk["id"]."\">".$_spk["id"]."</a>",
						"<a href=\"spk_ba_bayar_edit.php?id=".$_spk["id"]."\">".$_spk["ba_bayar_nomor"]."</a>",
						"<a href=\"spk_ba_bayar_edit.php?id=".$_spk["id"]."\">".format_tanggal($_spk["ba_bayar_tanggal"])."</a>",
						"<a href=\"spk_ba_bayar_edit.php?id=".$_spk["id"]."\">".$work_category."</a>",
                        "<a href=\"spk_ba_bayar_edit.php?id=".$_spk["id"]."\">".$procurement_work."</a>",
                        $ppk_name."<br>".$ppk_nip,
                        $tahun_anggaran,
                        $work_days,
