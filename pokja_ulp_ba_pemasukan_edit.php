...
<div class="bo_title">Ubah Berita Acara Pemasukan Penawaran Pengadaan</div>
<script>
	function download_file(pokja_ulp_id){
		window.open("pokja_ulp_ba_pemasukan_view.php?pokja_ulp_id=" + pokja_ulp_id);
	}
</script>
<?php
	if(isset($_POST["save"])){
		...
		$db->addfield("ba_pemasukan_nomor");		$db->addvalue($_POST["ba_pemasukan_nomor"]);
		$db->addfield("ba_pemasukan_tanggal");		$db->addvalue($_POST["ba_pemasukan_tanggal"]);
		$db->addfield("ba_pemasukan_updated_at");	$db->addvalue(date("Y-m-d H:i:s"));
		$db->addfield("ba_pemasukan_updated_by");	$db->addvalue($__username);
		$inserting = $db->update();
	...
	$txt_ba_pemasukan_nomor = $f->input("ba_pemasukan_nomor","","style='width:300px;'");
	$txt_ba_pemasukan_tanggal = $f->input("ba_pemasukan_tanggal","","type='date'");
...
<?=$f->start("","POST","","enctype='multipart/form-data'");?>
	<?=$t->start("","editor_content");?>
        ...
        <?=$t->row(array("Nomor Berita Acara Pemasukan Penawaran Pengadaan",$txt_ba_pemasukan_nomor));?>
        <?=$t->row(array("Tanggal Dokumen",$txt_ba_pemasukan_tanggal));?>
	...
	<?=$f->input("btn_ba_pembukaan","Buat BA Pembukaan Dokumen Penawaran","type='button' onclick='window.location=\"pokja_ulp_ba_pembukaan_add.php?pokja_ulp_id=".$_GET["id"]."\";'");?>
<?=$f->end();?>
<?php
	$ba_pemasukan_nomor = $db->fetch_single_data("pokja_ulp","ba_pemasukan_nomor",array("id" => $_GET["id"]));
	$ba_pemasukan_tanggal = $db->fetch_single_data("pokja_ulp","ba_pemasukan_tanggal",array("id" => $_GET["id"]));
...
<script>	
	...
	document.getElementById("ba_pemasukan_nomor").value = "<?=$ba_pemasukan_nomor;?>";
	document.getElementById("ba_pemasukan_tanggal").value = "<?=$ba_pemasukan_tanggal;?>";
