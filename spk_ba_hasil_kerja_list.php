...
<?php
	if($_GET["deleting"]){
		...
		if($db->fetch_single_data("spk","ba_serah_nomor",array("id"=>$_GET["deleting"])) == ""){
			$db->addtable("spk");						$db->where("id",$_GET["deleting"]);
			$db->addfield("ba_hasil_kerja_nomor");		$db->addvalue("");
			$db->addfield("ba_hasil_kerja_tanggal");	$db->addvalue("");
			$db->addfield("ba_hasil_kerja_nama_1");		$db->addvalue("");
			$db->addfield("ba_hasil_kerja_nip_1");		$db->addvalue("");
			$db->addfield("ba_hasil_kerja_nama_2");		$db->addvalue("");
			$db->addfield("ba_hasil_kerja_nip_2");		$db->addvalue("");
			$db->addfield("ba_hasil_kerja_nama_3");		$db->addvalue("");
			$db->addfield("ba_hasil_kerja_nip_3");		$db->addvalue("");
			$db->addfield("ba_hasil_kerja_updated_at");	$db->addvalue("");
			$db->addfield("ba_hasil_kerja_updated_by");	$db->addvalue("");
			$db->update();
...
?>
<div class="bo_title"> Berita Acara Pemeriksaan Penyelesaian Hasil Pekerjaan</div>
	...
	<div id="bo_filter_container">
		<?=$f->start("filter","GET");?>
			...
			<?php
				...
				$ba_hasil_kerja_nomor = $f->input("ba_hasil_kerja_nomor",$_GET["ba_hasil_kerja_nomor"]);
				$ba_hasil_kerja_tanggal = $f->input("ba_hasil_kerja_tanggal",$_GET["ba_hasil_kerja_tanggal"],"type='date'");
			?>
			     <?=$t->row(array("Nama Pekerjaan Pengadaan",$procurement_work_id));?>
			     <?=$t->row(array("Nomor Dokumen Berita Acara",$ba_hasil_kerja_nomor));?>
			     <?=$t->row(array("Tanggal Dokumen",$ba_hasil_kerja_tanggal));?>
...
<?php
	$whereclause = "ba_hasil_kerja_nomor <> '' AND ";
	if(@$_GET["procurement_work_id"]!="") 	$whereclause .= "procurement_work_id = '".$_GET["procurement_work_id"]."' AND ";
	if(@$_GET["ba_hasil_kerja_nomor"]!="") 	$whereclause .= "ba_hasil_kerja_nomor LIKE '%".$_GET["ba_hasil_kerja_nomor"]."%' AND ";
	if(@$_GET["ba_hasil_kerja_tanggal"]!="") 	$whereclause .= "ba_hasil_kerja_tanggal LIKE '%".$_GET["ba_hasil_kerja_tanggal"]."%' AND ";
...
?>
	...
	<?=$f->input("add","Tambah","type='button' onclick=\"window.location='spk_ba_hasil_kerja_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($spk as $no => $_spk){ ?>
		<?php
			$actions = "<a href=\"spk_ba_hasil_kerja_edit.php?id=".$_spk["id"]."\">Ubah</a> |
		...
		<?=$t->row(
					array($actions,
						$no+$start+1,
						"<a href=\"spk_ba_hasil_kerja_edit.php?id=".$_spk["id"]."\">".$_spk["id"]."</a>",
						"<a href=\"spk_ba_hasil_kerja_edit.php?id=".$_spk["id"]."\">".$_spk["ba_hasil_kerja_nomor"]."</a>",
						"<a href=\"spk_ba_hasil_kerja_edit.php?id=".$_spk["id"]."\">".format_tanggal($_spk["ba_hasil_kerja_tanggal"])."</a>",
						"<a href=\"spk_ba_hasil_kerja_edit.php?id=".$_spk["id"]."\">".$work_category."</a>",
                        "<a href=\"spk_ba_hasil_kerja_edit.php?id=".$_spk["id"]."\">".$procurement_work."</a>",
                        $ppk_name."<br>".$ppk_nip,
                        $tahun_anggaran,
                        $work_days,
