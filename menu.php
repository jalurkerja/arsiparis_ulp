						...
						<td align="left" valign="top" nowrap>
							...
							<nav id="primary_nav_wrap">
								<ul>
									<?php
										if($__isloggedin){
											$db->addtable("backoffice_menu"); $db->addfield("id,name,url"); $db->where("parent_id",0); $db->where("is_active","1"); $db->order("seqno");
											$arrmenu = $db->fetch_data(true);
											foreach($arrmenu as $menu){
												if($__menu_ids[$menu["id"]]){
													echo "<li class='bo_menu'><a href='".$menu["url"]."'>".$menu["name"]."</a>";
													$db->addtable("backoffice_menu"); $db->addfield("id,name,url"); $db->where("parent_id",$menu["id"]); $db->where("is_active","1"); $db->order("seqno");
