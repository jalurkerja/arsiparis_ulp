<?php
	...
	header("Content-Disposition: attachment;Filename=ba_evaluasi_penawaran_pengadaan.doc");
	...
	$arr1 = array();
	array_push($arr1,"{ba_evaluasi_nomor}");
	array_push($arr1,"{ba_evaluasi_tanggal}");
	...
	$arr2 = array();
	array_push($arr2,$db->fetch_single_data("pokja_ulp","ba_evaluasi_nomor",array("id"=>$pokja_ulp_id)));
	array_push($arr2,format_tanggal($db->fetch_single_data("pokja_ulp","ba_evaluasi_tanggal",array("id"=>$pokja_ulp_id))));
	...
	array_push($arr2,sebut_hari($db->fetch_single_data("pokja_ulp","ba_evaluasi_tanggal",array("id"=>$pokja_ulp_id))));
	array_push($arr2,sebut_tanggal($db->fetch_single_data("pokja_ulp","ba_evaluasi_tanggal",array("id"=>$pokja_ulp_id))));
	...
	echo str_replace($arr1,$arr2,read_file("htmls/doc_14_".$page.".html"));
?>