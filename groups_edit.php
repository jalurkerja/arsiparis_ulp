...
<div class="bo_title">Edit Groups</div>
<?php
	if(isset($_POST["save"])){
		$db->addtable("groups");$db->where("id",$_GET["id"]);
	...
	$db->addtable("groups");$db->where("id",$_GET["id"]);$db->limit(1);$unit = $db->fetch_data();
	$txt_name = $f->input("name",$unit["name"]);
	
?>
<?=$f->start("","POST","","enctype='multipart/form-data'");?>
	<?=$t->start("","editor_content");?>
        <?=$t->row(array("Groups Name",$txt_name));?>
	<?=$t->end();?>
