...
<?php
	if($_GET["deleting"]){
		...
		if($db->fetch_single_data("pokja_ulp","laporan_nomor",array("id"=>$_GET["deleting"])) == ""){
			$db->addtable("pokja_ulp");				$db->where("id",$_GET["deleting"]);
			$db->addfield("ba_hasil_nomor");		$db->addvalue("");
			$db->addfield("ba_hasil_tanggal");		$db->addvalue("");
			$db->addfield("ba_hasil_supplier_id");	$db->addvalue("");
			$db->addfield("ba_hasil_updated_at");	$db->addvalue("");
			$db->addfield("ba_hasil_updated_by");	$db->addvalue("");
			$db->update();
...
?>
<div class="bo_title">Berita Acara Hasil Pengadaan</div>
	...
	<div id="bo_filter_container">
		<?=$f->start("filter","GET");?>
			...
			<?php
				...
				$ba_hasil_nomor = $f->input("ba_hasil_nomor",$_GET["ba_hasil_nomor"]);
				$ba_hasil_tanggal = $f->input("ba_hasil_tanggal",$_GET["ba_hasil_tanggal"],"type='date'");
			?>
			     <?=$t->row(array("Nama Pekerjaan Pengadaan",$procurement_work_id));?>
			     <?=$t->row(array("Nomor Undangan",$ba_hasil_nomor));?>
			     <?=$t->row(array("Tanggal Undangan",$ba_hasil_tanggal));?>
...
<?php
	$whereclause = "ba_hasil_nomor <> '' AND ";
	if(@$_GET["procurement_work_id"]!="") 	$whereclause .= "procurement_work_id = '".$_GET["procurement_work_id"]."' AND ";
	if(@$_GET["ba_hasil_nomor"]!="") 	$whereclause .= "ba_hasil_nomor LIKE '%".$_GET["ba_hasil_nomor"]."%' AND ";
	if(@$_GET["ba_hasil_tanggal"]!="") 	$whereclause .= "ba_hasil_tanggal LIKE '%".$_GET["ba_hasil_tanggal"]."%' AND ";
...
?>
	...
	<?=$f->input("add","Tambah","type='button' onclick=\"window.location='pokja_ulp_ba_hasil_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($pokja_ulp as $no => $_pokja_ulp){ ?>
		<?php
			$actions = "<a href=\"pokja_ulp_ba_hasil_edit.php?id=".$_pokja_ulp["id"]."\">Ubah</a> |
		...
		<?=$t->row(
					array($actions,
						$no+$start+1,
						"<a href=\"pokja_ulp_ba_hasil_edit.php?id=".$_pokja_ulp["id"]."\">".$_pokja_ulp["id"]."</a>",
						"<a href=\"pokja_ulp_ba_hasil_edit.php?id=".$_pokja_ulp["id"]."\">".$_pokja_ulp["ba_hasil_nomor"]."</a>",
						"<a href=\"pokja_ulp_ba_hasil_edit.php?id=".$_pokja_ulp["id"]."\">".format_tanggal($_pokja_ulp["ba_hasil_tanggal"])."</a>",
						"<a href=\"pokja_ulp_ba_hasil_edit.php?id=".$_pokja_ulp["id"]."\">".$work_category."</a>",
                        "<a href=\"pokja_ulp_ba_hasil_edit.php?id=".$_pokja_ulp["id"]."\">".$procurement_work."</a>",
                        $ppk_name."<br>".$ppk_nip,
                        $tahun_anggaran,
                        $work_days,
