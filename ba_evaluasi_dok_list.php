...
<?php
	if($_GET["deleting"]){
		$procurement_work_id = $db->fetch_single_data("ba_evaluasi_dok","procurement_work_id",array("id"=>$_GET["deleting"]));
		if($db->fetch_single_data("pokja_ulp","nego_nomor",array("procurement_work_id"=>$procurement_work_id)) == ""){
...
?>
<div class="bo_title">Berita Acara Evaluasi dan Penelitian Dokumen Penawaran</div>
...
?>
	...
	<?=$f->input("add","Tambah","type='button' onclick=\"window.location='ba_evaluasi_dok_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($data as $no => $_data){ ?>
		<?php
			$actions = "<a href=\"ba_evaluasi_dok_edit.php?id=".$_data["id"]."\">Ubah</a> |
		...
		<?=$t->row(
					array($actions,
						$no+$start+1,
						"<a href=\"ba_evaluasi_dok_edit.php?id=".$_data["id"]."\">".$_data["id"]."</a>",
						"<a href=\"ba_evaluasi_dok_edit.php?id=".$_data["id"]."\">".$_data["nomor"]."</a>",
						"<a href=\"ba_evaluasi_dok_edit.php?id=".$_data["id"]."\">".format_tanggal($_data["tanggal"])."</a>",
						"<a href=\"ba_evaluasi_dok_edit.php?id=".$_data["id"]."\">".$work_category."</a>",
                        "<a href=\"ba_evaluasi_dok_edit.php?id=".$_data["id"]."\">".$procurement_work."</a>",
                        $ppk_name."<br>".$ppk_nip,
                        $tahun_anggaran,
                        $work_days,
