...
<div class="bo_title">Ubah Berita Acara Evaluasi Penawaran</div>
<script>
	function download_file(pokja_ulp_id){
		window.open("pokja_ulp_ba_evaluasi_view.php?pokja_ulp_id=" + pokja_ulp_id);
	}
</script>
<?php
	if(isset($_POST["save"])){
		...
		$db->addfield("ba_evaluasi_nomor");			$db->addvalue($_POST["ba_evaluasi_nomor"]);
		$db->addfield("ba_evaluasi_tanggal");		$db->addvalue($_POST["ba_evaluasi_tanggal"]);
		$db->addfield("ba_evaluasi_updated_at");	$db->addvalue(date("Y-m-d H:i:s"));
		$db->addfield("ba_evaluasi_updated_by");	$db->addvalue($__username);
		$inserting = $db->update();
	...
	$txt_ba_evaluasi_nomor = $f->input("ba_evaluasi_nomor","","style='width:300px;'");
	$txt_ba_evaluasi_tanggal = $f->input("ba_evaluasi_tanggal","","type='date'");
...
<?=$f->start("","POST","","enctype='multipart/form-data'");?>
	<?=$t->start("","editor_content");?>
        ...
        <?=$t->row(array("Nomor Dokumen",$txt_ba_evaluasi_nomor));?>
        <?=$t->row(array("Tanggal Dokumen",$txt_ba_evaluasi_tanggal));?>
...
<?=$f->end();?>
<?php
	$ba_evaluasi_nomor = $db->fetch_single_data("pokja_ulp","ba_evaluasi_nomor",array("id" => $_GET["id"]));
	$ba_evaluasi_tanggal = $db->fetch_single_data("pokja_ulp","ba_evaluasi_tanggal",array("id" => $_GET["id"]));
...
<script>	
	...
	document.getElementById("ba_evaluasi_nomor").value = "<?=$ba_evaluasi_nomor;?>";
	document.getElementById("ba_evaluasi_tanggal").value = "<?=$ba_evaluasi_tanggal;?>";
