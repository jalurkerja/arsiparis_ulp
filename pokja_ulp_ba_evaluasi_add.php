...
<div class="bo_title">Tambah Berita Acara Evaluasi Penawaran</div>
<?php
	if($_GET["pokja_ulp_id"] > 0){
		...
		if($db->fetch_single_data("pokja_ulp","ba_evaluasi_nomor",array("id"=>$_GET["pokja_ulp_id"])) > 0)
			javascript("window.location='pokja_ulp_ba_evaluasi_edit.php?id=".$_GET["pokja_ulp_id"]."';");
	}
	if(isset($_POST["save"])){
		...
		if($pokja_ulp_id > 0){
			...
			if($inserting["affected_rows"] >= 0){
				...
				javascript("window.location='pokja_ulp_ba_evaluasi_edit.php?id=".$pokja_ulp_id."';");
			} else {
