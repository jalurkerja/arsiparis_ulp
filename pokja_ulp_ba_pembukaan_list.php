...
<?php
	if($_GET["deleting"]){
		...
		if($db->fetch_single_data("ba_evaluasi_dok","id",array("procurement_work_id"=>$procurement_work_id)) == ""){
			$db->addtable("pokja_ulp");					$db->where("id",$_GET["deleting"]);
			$db->addfield("ba_pembukaan_nomor");		$db->addvalue("");
			$db->addfield("ba_pembukaan_tanggal");		$db->addvalue("");
			$db->addfield("ba_pembukaan_updated_at");	$db->addvalue("");
			$db->addfield("ba_pembukaan_updated_by");	$db->addvalue("");
			$db->update();
...
?>
<div class="bo_title">Berita Acara Pembukaan Dokumen Penawaran</div>
	...
	<div id="bo_filter_container">
		<?=$f->start("filter","GET");?>
			...
			<?php
				...
				$ba_pembukaan_nomor = $f->input("ba_pembukaan_nomor",$_GET["ba_pembukaan_nomor"]);
				$ba_pembukaan_tanggal = $f->input("ba_pembukaan_tanggal",$_GET["ba_pembukaan_tanggal"],"type='date'");
			?>
			     <?=$t->row(array("Nama Pekerjaan Pengadaan",$procurement_work_id));?>
			     <?=$t->row(array("Nomor Dokumen Berita Acara",$ba_pembukaan_nomor));?>
			     <?=$t->row(array("Tanggal Dokumen",$ba_pembukaan_tanggal));?>
...
<?php
	$whereclause = "ba_pembukaan_nomor <> '' AND ";
	if(@$_GET["procurement_work_id"]!="") 	$whereclause .= "procurement_work_id = '".$_GET["procurement_work_id"]."' AND ";
	if(@$_GET["ba_pembukaan_nomor"]!="") 	$whereclause .= "ba_pembukaan_nomor LIKE '%".$_GET["ba_pembukaan_nomor"]."%' AND ";
	if(@$_GET["ba_pembukaan_tanggal"]!="") 	$whereclause .= "ba_pembukaan_tanggal LIKE '%".$_GET["ba_pembukaan_tanggal"]."%' AND ";
...
?>
	...
	<?=$f->input("add","Tambah","type='button' onclick=\"window.location='pokja_ulp_ba_pembukaan_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($pokja_ulp as $no => $_pokja_ulp){ ?>
		<?php
			$actions = "<a href=\"pokja_ulp_ba_pembukaan_edit.php?id=".$_pokja_ulp["id"]."\">Ubah</a> |
		...
		<?=$t->row(
					array($actions,
						$no+$start+1,
						"<a href=\"pokja_ulp_ba_pembukaan_edit.php?id=".$_pokja_ulp["id"]."\">".$_pokja_ulp["id"]."</a>",
						"<a href=\"pokja_ulp_ba_pembukaan_edit.php?id=".$_pokja_ulp["id"]."\">".$_pokja_ulp["ba_pembukaan_nomor"]."</a>",
						"<a href=\"pokja_ulp_ba_pembukaan_edit.php?id=".$_pokja_ulp["id"]."\">".format_tanggal($_pokja_ulp["ba_pembukaan_tanggal"])."</a>",
						"<a href=\"pokja_ulp_ba_pembukaan_edit.php?id=".$_pokja_ulp["id"]."\">".$work_category."</a>",
                        "<a href=\"pokja_ulp_ba_pembukaan_edit.php?id=".$_pokja_ulp["id"]."\">".$procurement_work."</a>",
                        $ppk_name."<br>".$ppk_nip,
                        $tahun_anggaran,
                        $work_days,
