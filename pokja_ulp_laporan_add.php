...
<div class="bo_title">Tambah Laporan Hasil Pengadaan</div>
<?php
	if($_GET["pokja_ulp_id"] > 0){
		...
		if($db->fetch_single_data("pokja_ulp","laporan_nomor",array("id"=>$_GET["pokja_ulp_id"])) > 0)
			javascript("window.location='pokja_ulp_laporan_edit.php?id=".$_GET["pokja_ulp_id"]."';");
	}
	if(isset($_POST["save"])){
		...
		if($pokja_ulp_id > 0){
			...
			if($inserting["affected_rows"] >= 0){
				...
				javascript("window.location='pokja_ulp_laporan_edit.php?id=".$pokja_ulp_id."';");
			} else {
