...
<div class="bo_title">Ubah Berita Acara Evaluasi dan Penelitian Dokumen Penawaran</div>
...
<?php
	if(isset($_POST["save"])){
		$db->addtable("ba_evaluasi_dok"); $db->where("procurement_work_id",$procurement_work_id); $db->delete_();
		$ba_evaluasi_dok_id = "";
		...
		javascript("window.location='?id=".$ba_evaluasi_dok_id."';");
	}
...
<?=$f->start("","POST","","enctype='multipart/form-data'");?>
	...
	<?=$f->input("btn_penawaran","Buat Surat Undangan Negosiasi","type='button' onclick='window.location=\"pokja_ulp_nego_add.php?pokja_ulp_id=".$pokja_ulp_id."\";'");?>
<?=$f->end();?>
<?php
	$nomor = $db->fetch_single_data("ba_evaluasi_dok","nomor",array("id" => $_GET["id"]));
	$tanggal = $db->fetch_single_data("ba_evaluasi_dok","tanggal",array("id" => $_GET["id"]));
