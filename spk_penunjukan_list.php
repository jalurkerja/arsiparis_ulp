...
<?php
	if($_GET["deleting"]){
		if($db->fetch_single_data("spk","pemeriksaan_nomor",array("id"=>$_GET["deleting"])) == ""){
			$db->addtable("spk");
...
?>
<div class="bo_title">Surat Penunjukan Penyedia Barang/Jasa</div>
	...
	<div id="bo_filter_container">
		<?=$f->start("filter","GET");?>
			...
			<?php
				...
				$penunjukan_nomor = $f->input("penunjukan_nomor",$_GET["penunjukan_nomor"]);
				$penunjukan_tanggal = $f->input("penunjukan_tanggal",$_GET["penunjukan_tanggal"],"type='date'");
			...
			?>
			     ...
			     <?=$t->row(array("Nomor Surat Penunjukan",$penunjukan_nomor));?>
			     <?=$t->row(array("Tanggal Surat Penunjukan",$penunjukan_tanggal));?>
...
<?php
	$whereclause = "penunjukan_nomor <> '' AND ";
	if(@$_GET["procurement_work_id"]!="") 		$whereclause .= "procurement_work_id = '".$_GET["procurement_work_id"]."' AND ";
	if(@$_GET["penunjukan_nomor"]!="") 			$whereclause .= "penunjukan_nomor LIKE '%".$_GET["penunjukan_nomor"]."%' AND ";
	if(@$_GET["penunjukan_tanggal"]!="") 		$whereclause .= "penunjukan_tanggal LIKE '%".$_GET["penunjukan_tanggal"]."%' AND ";
...
?>
	...
	<?=$f->input("add","Tambah","type='button' onclick=\"window.location='spk_penunjukan_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($spk as $no => $_spk){ ?>
		<?php
			$actions = "<a href=\"spk_penunjukan_edit.php?id=".$_spk["id"]."\">Ubah</a> |
		...
		<?=$t->row(
					array($actions,
						$no+$start+1,
						"<a href=\"spk_penunjukan_edit.php?id=".$_spk["id"]."\">".$_spk["id"]."</a>",
						"<a href=\"spk_penunjukan_edit.php?id=".$_spk["id"]."\">".$_spk["penunjukan_nomor"]."</a>",
						"<a href=\"spk_penunjukan_edit.php?id=".$_spk["id"]."\">".format_tanggal($_spk["penunjukan_tanggal"])."</a>",
						"<a href=\"spk_penunjukan_edit.php?id=".$_spk["id"]."\">".$work_category."</a>",
                        "<a href=\"spk_penunjukan_edit.php?id=".$_spk["id"]."\">".$procurement_work."</a>",
                        $ppk_name."<br>".$ppk_nip,
                        $tahun_anggaran,
                        $work_days,
                        format_amount($hps_nominal),
						format_amount($_spk["penawaran_harga"])),
