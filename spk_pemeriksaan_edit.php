...
<div class="bo_title">Ubah Permintaan Pemeriksaan Hasil Pengadaan Barang/Jasa</div>
<script>
	function download_file(spk_id){
		window.open("spk_pemeriksaan_view.php?spk_id=" + spk_id);
	}
</script>
<?php
	if(isset($_POST["save"])){
		...
		$db->addfield("pemeriksaan_nomor");				$db->addvalue($_POST["pemeriksaan_nomor"]);
		$db->addfield("pemeriksaan_tanggal");			$db->addvalue($_POST["pemeriksaan_tanggal"]);
		$db->addfield("pemeriksaan_supplier_nomor");	$db->addvalue($_POST["pemeriksaan_supplier_nomor"]);
		$db->addfield("pemeriksaan_supplier_tanggal");	$db->addvalue($_POST["pemeriksaan_supplier_tanggal"]);
		$db->addfield("pemeriksaan_updated_at");		$db->addvalue(date("Y-m-d H:i:s"));
		$db->addfield("pemeriksaan_updated_by");		$db->addvalue($__username);
		$inserting = $db->update();
	...
	$txt_pemeriksaan_nomor = $f->input("pemeriksaan_nomor","","style='width:300px;'");
	$txt_pemeriksaan_tanggal = $f->input("pemeriksaan_tanggal","","type='date'");
	$txt_pemeriksaan_supplier_nomor = $f->input("pemeriksaan_supplier_nomor","","style='width:300px;'");
	$txt_pemeriksaan_supplier_tanggal = $f->input("pemeriksaan_supplier_tanggal","","type='date'");
...
<?=$f->start("","POST","","enctype='multipart/form-data'");?>
	<?=$t->start("","editor_content");?>
        ...
        <?=$t->row(array("Nomor Permintaan Pemeriksaan Hasil Pengadaan Barang/Jasa",$txt_pemeriksaan_nomor));?>
        <?=$t->row(array("Tanggal Dokumen",$txt_pemeriksaan_tanggal));?>
        <?=$t->row(array("Penyedia Barang/Jasa","<div id='supplier_name' ".$datastyle."></div>"));?>
        <?=$t->row(array("Nomor Surat dari Penyedia Barang/Jasa",$txt_pemeriksaan_supplier_nomor));?>
        <?=$t->row(array("Tanggal Surat dari Penyedia Barang/Jasa",$txt_pemeriksaan_supplier_tanggal));?>
	...
	<?=$f->input("btn_ba_pemeriksaan","Buat BA Pemeriksaan Hasil Pekerjaan","type='button' onclick='window.location=\"spk_ba_hasil_kerja_add.php?spk_id=".$_GET["id"]."\";'");?>
<?=$f->end();?>
<?php
	$pemeriksaan_nomor = $db->fetch_single_data("spk","pemeriksaan_nomor",array("id" => $_GET["id"]));
	$pemeriksaan_tanggal = $db->fetch_single_data("spk","pemeriksaan_tanggal",array("id" => $_GET["id"]));
	$pemeriksaan_supplier_nomor = $db->fetch_single_data("spk","pemeriksaan_supplier_nomor",array("id" => $_GET["id"]));
	$pemeriksaan_supplier_tanggal = $db->fetch_single_data("spk","pemeriksaan_supplier_tanggal",array("id" => $_GET["id"]));
...
<script>	
	...
	document.getElementById("pemeriksaan_nomor").value = "<?=$pemeriksaan_nomor;?>";
	document.getElementById("pemeriksaan_tanggal").value = "<?=$pemeriksaan_tanggal;?>";
	document.getElementById("pemeriksaan_supplier_nomor").value = "<?=$pemeriksaan_supplier_nomor;?>";
	document.getElementById("pemeriksaan_supplier_tanggal").value = "<?=$pemeriksaan_supplier_tanggal;?>";
